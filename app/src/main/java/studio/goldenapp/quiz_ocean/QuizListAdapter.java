...
public class QuizListAdapter extends RecyclerView.Adapter<QuizListAdapter.QuizViewHolder> {
    ...
    public void onBindViewHolder(@NonNull QuizViewHolder holder, int position) {
        ...
        if(listDescription.length() > 80){
            listDescription = listDescription.substring(0, 60);
        }

        holder.listDesc.setText(String.format("%s...", listDescription));
        holder.listLevel.setText(quizListModels.get(position).getLevel());
        holder.listLevel.setPaintFlags(holder.listLevel.getPaintFlags() |   Paint.UNDERLINE_TEXT_FLAG);
